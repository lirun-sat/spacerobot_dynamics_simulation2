...
def calc_hh(R0, RR, A0, AA):
    ...
    :param R0: position of the base
    :param RR: position of each link
    :param A0: attitude of the base
    :param AA: attitude of each link
    :return: inertia matrices: H
    """
    JJ_t = calc_jt(RR, AA)
    JJ_r = calc_jr(AA)

    m_tot_E = mass * np.eye(3)

    JJ_tg = np.zeros((num_q, 3))
    # submatrix JJ_tg collects the contribution to the system kinetic energy
    # of the combination of the effect of the manipulator joint rate \dot{q} and the base-spacecraft linear velocity v0

    HH_w = np.zeros((3, 3))
    # submatrix HH_w collects the moments-of-inertia of the spacecraft-manipulator system about the base-spacecraft center-of-mass

    HH_wq = np.zeros((num_q, 3))
    # submatrix HH_wq contains the contribution to the system kinetic energy
    # of the combination of the effect of the manipulator joint rate \dot{q} and the base-spacecraft angular velocity ω0

    HH_q = np.zeros((num_q, num_q))
    # The [N×N] inertia matrix for the manipulator, HH_q, is identical to that of any fixed-base manipulator

    HH = np.zeros((num_q+6, num_q+6))
    m_tot_r0g = np.zeros(3)
    ...
    HH_w += A0 @ inertia0 @ A0.T
    HH[0:3, 0:3] = m_tot_E
    HH[0:3, 3:6] = skew_sym(m_tot_r0g).T
    HH[0:3, 6:6+num_q] = JJ_tg.T
    HH[3:6, 0:3] = skew_sym(m_tot_r0g)
    HH[3:6, 3:6] = HH_w
